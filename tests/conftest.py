...
def tiny_cfg() -> VajraConfig:
    ...
    from vajra.config import DecoderConfig, EncoderConfig, FusionConfig

    cfg = VajraConfig()
    cfg.shared_d_model = 64
    cfg.domain_encoders = {
        "detection_network": EncoderConfig(
            layers=2, d_model=64, attention_heads=4, ffn_width=256, dcat_layers=[1]
        ),
        "forensics_provenance": EncoderConfig(
            layers=2, d_model=64, attention_heads=4, ffn_width=256, dcat_layers=[1]
        ),
        "cti_stix": EncoderConfig(
            layers=2, d_model=64, attention_heads=4, ffn_width=256
        ),
    ...
    cfg.decoder = DecoderConfig(
        layers=2, d_model=64, attention_heads=4, ffn_width=256, max_output_tokens=16
    )
    return cfg
