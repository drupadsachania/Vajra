...
class BaselineCache:
    ...
    def update(
        self, new_state: torch.Tensor, decision_class: int | None = None
    ) -> torch.Tensor:
        ...
        if self._cache is None or self._cache.shape != new_state.shape:
            ...
            self._cache = self.BETA * self._cache + (1 - self.BETA) * new_state.detach()
        return self._cache
...
class DCATBlock(nn.Module):
    ...
    def forward(
        ...
        output = self.final_ln(
            x_combined + self.dropout(self.ffn(self.ln2(x_combined)))
        )
